...
class SoundPlayer(threading.Thread):
    ...
    def create_sound(self,soundParams):
        ...
        if soundParams['type']=='tone':
            ...
            return(soundObj,[envelope,soundwaveObj])
            '''
        elif soundParams['type']=='gapinnoise':
            gaponset =soundParams['gaponset'] 
            gapdur=soundParams['gapdur']/1000 
            duration=soundParams['duration']
            amp=soundAmp[1]
            envelope = pyo.Linseg([(0, amp),(gaponset, amp),(gaponset,0),(gaponset+gapdur,0),(gaponset+gapdur, amp), (duration, amp)]).out()
            soundObj=pyo.Fader(fadein=self.risetime, fadeout=self.falltime,
                                 dur=soundParams['duration'], mul=envelope.mix(2))
            soundwaveObj = pyo.Noise(mul=soundObj).out()
            return(soundObj,soundwaveObj)
